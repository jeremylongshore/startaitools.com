...
def test_blogposting_schema_is_deterministic_and_deduped():
    ...
    page, standing down only when a body already carries an inline BlogPosting
    block, so no page ever serves two BlogPosting declarations. A body that
    carries only a FAQPage block (the SEO phase's job) must still get one."""
    t = SCHEMA_PARTIAL.read_text(encoding="utf-8")
    assert '"@type": "BlogPosting"' in t
    assert 'in .RawContent "\\"BlogPosting\\""' in t, "dedupe guard missing"
    assert 'in .RawContent "application/ld+json"' not in t, (
        "guard must not stand down for FAQPage-only bodies")
