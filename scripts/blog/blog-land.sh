...
# ---- Ezekiel posting packet (WS2 hook — no-op until the script exists) -------
PACKET="$(dirname "${BASH_SOURCE[0]}")/blog-posting-packet.sh"
if [ -x "$PACKET" ]; then
  log "Building Ezekiel posting packet for $SLUG..."
  "$PACKET" "$TARGET_DATE" >> "$LOG" 2>&1 || log "WARN: posting-packet returned non-zero"
else
  log "NOTE: blog-posting-packet.sh not present yet — skipping packet (will wire in WS2)."
fi
