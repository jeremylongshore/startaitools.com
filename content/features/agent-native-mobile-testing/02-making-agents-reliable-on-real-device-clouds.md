...
**TL;DR.** Chapter 1 found the gaps an `AGENTS.md` should close: lifecycle invisibility, endpoint compatibility, and the cooldown window where a device reports `is_online=true` but cannot take a session yet. Chapter 2 builds the reference implementation that closes them. The reliability problem on a real-device cloud is not the API calls. It is the **partial-failure moments that emerge between** MCP tool calls. The fix is making those seams legible: three task-specific agent definitions, four advisory hooks, and platform limitations documented as a contract rather than discovered the expensive way.

---

Chapter 1 ran a 5-device parity sweep through [`kobiton/automate`](https://github.com/kobiton/automate) and came away with a list of operational facts that an agent would otherwise learn by getting burned: which Appium log endpoints work, what "online" actually means, and how long a device sulks after `deleteSession`. The conclusion was that those gaps are documentation, not code.
...
- **Known limitations documented as partnership:** the cooldown window, endpoint compatibility boundaries, and lifecycle states between `online` and `session-ready`, written down as a contract the agent can read. This is precisely the `AGENTS.md` surface Chapter 1 argued for.
...
The complete write-up includes the worked partial-failure sequence, the three routed decisions, the four hook signals, and "known limitations as partnership." It runs as the daily post, which mirrors the canonical version on Kobiton's engineering blog:
