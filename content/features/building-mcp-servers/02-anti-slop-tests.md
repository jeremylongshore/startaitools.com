...
The post linked above documents three real bugs the framework caught when it was first turned loose on its own implementation. The most embarrassing one was a "happy path" test that asserted the wrong field. It passed for two months before the framework's own escape-scan flagged it.
