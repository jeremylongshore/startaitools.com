...
slug = 'guidewire-mcp-v0-1-0-v0-1-1-76-minutes'
date = 2026-05-06T20:30:00-06:00
draft = false
...
toc = true
tldr = "v0.1.0 of @intentsolutions/policycenter-mcp shipped read-only carrier-vocabulary tools to npm at 19:14 Mountain. Within 76 minutes an install-path defect surfaced — the prepare hook silently swallowed build failures via `|| true`. v0.1.1 went out at 20:30. The v0.1.1 version bump rewrote 8 package.json files via jq, breaking biome lint; a same-evening cleanup commit reconciled the formatting. The hardest single technical takeaway: the harness needs both a writer and a reader Postgres pool, because a hash-chained audit log only gives you tamper-evidence if the verification path binds via a role that physically cannot write. Two releases, one cleanup, full audit trail, all in one evening. The discipline IS the product."
+++
