...
slug = 'the-drills-passed-reality-did-not'
...
Then the drills lied. Three separate times. This is a recurring theme around here: two days earlier, [a CI drift watch printed a green check for a check it had silently skipped](/posts/the-day-the-green-checks-were-lying/).
...
The first one is the worst, and it is worth sitting with. Without an explicit `--sec`, `nak` signs with its own machine-default key rather than the member key the probe just created. The probe was authenticating as the wrong identity and reporting green. A test that passes for the wrong reason is more dangerous than one that fails, because it spends its whole life telling you a thing you never actually verified. [The same root cause has produced false positives here before](/posts/two-false-positive-fixes-same-root-cause/).
...
- [The Day The Green Checks Were Lying](/posts/the-day-the-green-checks-were-lying/)
- [Two False Positive Fixes, Same Root Cause](/posts/two-false-positive-fixes-same-root-cause/)
