...
slug = 'a-path-is-not-proof-of-identity'
...
This is the second time this bug class has come up here. [CodeQL caught the race I dismissed](/posts/codeql-caught-the-race-i-dismissed/) covers the same shape in TypeScript, where the fix was a same-descriptor `openSync`, `fstatSync`, `writeSync` chain. Different language, identical lesson.
