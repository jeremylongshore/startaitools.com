...
slug = "move-wrapper-math-out-of-the-llm-prompt"
date = 2026-09-23T10:00:00-06:00
draft = false
...
tldr = "A wrapper's deterministic gate lives in the files it reads, not in its prompt. Materialize the date window, the site list, and the per-site totals in a Python file before the prompt is built, then run --check-report against the registry before sending. If any domain is missing from the final HTML, the email does not send."
+++
...
- [Deterministic-First LLM Advisory CI](/posts/deterministic-first-llm-advisory-ci/)
- [Git Plumbing for Unattended Cron in a Shared Checkout](/posts/git-plumbing-unattended-cron-shared-checkout/)
- [Backlog Zero: The Estate Triage Machine](/posts/backlog-zero-estate-triage-machine/)