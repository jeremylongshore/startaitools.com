...
slug = 'three-classes-of-self-healing-and-slow-is-not-failed'
...
tldr = "An unattended AI pipeline needs three classes of self-healing to actually be unattended: repair rounds for transient gate failures, depth-capped provider failover for dead LLMs, and nightly catch-up of missed dates. The meta-lesson is that slow is not failed: a deploy that takes 25 minutes deserves PENDING, not a 4am page. Independent review of the catch-up code caught exactly the case where the failover parent was reading a PENDING failover as failover also failed and paging for a night that succeeded. The very noise this change exists to remove."
+++
...
- [The Pipeline Landed Its Own Output Today](/posts/the-pipeline-landed-its-own-output-today/)
- [The BitLocker Helper Lifetime Receipt Pack Ran a Full Day](/posts/the-helper-lifetime-receipt-pack-ran-for-a-full-day/)
- [Reachability Is Not Freshness](/posts/reachability-is-not-freshness/)
- [Seven Merges, One Audit, Zero Leaks](/posts/seven-merges-one-audit-zero-leaks/)