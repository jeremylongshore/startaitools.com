...
slug = 'three-gates-on-standard-user-bitlocker-state-observation'
date = 2026-09-13T08:00:00-05:00
draft = false
...
tldr = "Closing intent-blue-gold Epic 1+2 introduced three gates that have to hold together for standard-user BitLocker state observation to be safe: the observer contract, the bounded native provider, and the fixture-boundary authorization. A Codex (GPT-5.6 Sol) read-only audit on merge day surfaced three P1 failures inside the fixture-boundary authorization: a CLI that admitted arbitrary temp roots against the canonical fixture, a path-based directory traversal vulnerable to TOCTOU reparse replacement, and a stale offline/recall classification that let non-reparse offline files register as local. The audit did not find P0s and two of the three P1s were exactly the kind of failure it was built to catch."
+++
