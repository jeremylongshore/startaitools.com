...
slug = 'git-plumbing-unattended-cron-shared-checkout'
date = 2026-09-21T10:00:00-06:00
draft = false
...
tldr = "An unattended cron that has to commit to a shared, often-dirty checkout should not use porcelain (git add, git commit, git push from inside the repo). The fix is plumbing: hash-object the new file, read-tree the fresh origin tip, write-tree, commit-tree, then push <new-sha>:branch. The working tree is never touched, so a concurrent session cannot break it, and a stale local main cannot reject the push. The mechanism that keeps startaitools mirroring daily posts to a sister site at 4am is exactly this."
+++
...
- [Three Self-Healing Classes for an Unattended AI Pipeline](/posts/three-classes-of-self-healing-and-slow-is-not-failed/) (2026-09-20): the dual-publish is one mechanism in the same unattended-pipeline system; that post names the three classes the daily cron sits inside.
- [The Pipeline Landed Its Own Output Today](/posts/the-pipeline-landed-its-own-output-today/) (2026-09-15): earlier post about the blog pipeline producing and landing its own output.
- [Reachability Is Not Freshness](/posts/reachability-is-not-freshness/) (2026-09-17): diagnostic post about a status page that returned 200 while serving stale data; mirrors the "indicator that passes while the system is broken" theme at a different layer.