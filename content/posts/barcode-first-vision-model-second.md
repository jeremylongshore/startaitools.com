...
slug = 'barcode-first-vision-model-second'
...
tldr = "Research before the build said the same thing every working incumbent says: an LLM cannot be the identifier for issue-exact and variant-exact comic ID. So the identifier seat went to the barcode and the retrieval step, and the model got the re-rank seat behind a gate that cross-checks the evidence it claims to have read against candidate metadata. A contradiction downgrades the confidence band and forces a human pick. The database enforces the audit trail itself with a trigger that raises on UPDATE and DELETE, because code discipline only holds until someone opens psql."
+++
...
So the pipeline puts the [deterministic parts first](/posts/llm-legible-deterministic-architecture/).
...
- [Shipping a CAD Agent from Zero: DXF Parsing, Edit Engines, and LLM Planner Interfaces](/posts/building-cad-dxf-agent-from-zero-to-v010/)
- [Every Safety Gate Has a Failure Direction](/posts/every-safety-gate-has-a-failure-direction/)
- [Noise-Robust LLM-Judge Evals: Don't Sign a Coin Flip](/posts/noise-robust-signed-llm-judge-evals/)
