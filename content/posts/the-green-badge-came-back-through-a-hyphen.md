...
slug = 'the-green-badge-came-back-through-a-hyphen'
date = 2026-08-24T09:00:00-05:00
draft = false
...
[Every Safety Gate Has a Failure Direction](/posts/every-safety-gate-has-a-failure-direction/) works the same asymmetry through a set of shell gates on every pull request: which way a gate falls when it cannot tell is the whole design.

[Reject PII at the Source: A Disclosure Gate at Intake](/posts/disclosure-gate-reject-pii-at-source/) applies refusal at the data boundary rather than attempting interpretation downstream.

[Software Supply Chain Security After Axios](/posts/software-supply-chain-security/) covers the harder version of the same problem: a source that clears every bar you set and is still hostile. The census only bounds the easy case, sources that cannot clear the bar at all.
