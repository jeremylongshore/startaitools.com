...
slug = 'a-dead-socket-is-not-a-dead-host'
date = 2026-08-10T08:00:00-05:00
draft = false
...
[The Check That Only Confirmed a Name](/posts/the-check-that-only-confirmed-a-name/). A gate that validated the label instead of the thing behind it. The F2 finding here is the same shape.

[Nothing Read It, So Nothing Failed](/posts/nothing-read-it-so-nothing-failed/). What it costs when absence and success are indistinguishable to the consumer.

[The Ghost in the Catalog](/posts/the-ghost-in-the-catalog/). A record asserting work that never happened, then read downstream as truth. The same collapse, applied to provenance.
