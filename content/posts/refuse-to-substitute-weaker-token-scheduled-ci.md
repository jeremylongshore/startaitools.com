...
slug = 'refuse-to-substitute-weaker-token-scheduled-ci'
date = 2026-09-22T10:00:00-06:00
draft = false
...
tldr = 'PR #1563 lands a one-line fix in update-npm-stats.yml: refuse to start when the scheduled fine-grained PAT is absent. The workflow had been silently falling back to GITHUB_TOKEN on PAT expiry, which let it open PRs that could not re-trigger required checks. Branch protection said checks required while no required check ever ran. The fix is the absence of a fallback path.'
+++
...
- [Reachability Is Not Freshness](/posts/reachability-is-not-freshness/) (2026-09-17): a status page that 200'd while serving two-month-old data.
- [Wrong-Mode Green Is Not a Gate](/posts/wrong-mode-green-is-not-a-gate/) (2026-07-22): a freshness gate that accepted the wrong mode flag and printed all-green.
- [Git Plumbing for an Unattended Cron on a Shared Checkout](/posts/git-plumbing-unattended-cron-shared-checkout/) (2026-09-21): same unattended-pipeline theme, different failure mode.
