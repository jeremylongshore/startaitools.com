...
slug = 'postgres-approval-sink-bugs-the-tests-caught'
...
toc = true
tldr = "The 11 testcontainers tests for a Postgres-backed approval sink failed on first run. Two bugs surfaced before any prod deploy: PG 15+ removed implicit USAGE on the public schema, silently no-op'ing the migration's table GRANTs; and the audit_writer role needed SELECT on approvals because the sink drives a state machine and wait() must read its own writes — an asymmetry vs the audit_entries pattern. Both fixes shipped in PR #92 (+1581 / -2)."
+++
