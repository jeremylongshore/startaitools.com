...
slug = 'gate-the-statement-not-the-tool-name'
...
tldr = "One MCP tool carries every SQL verb — SELECT, DROP, CALL DOLT_PUSH — so allowlisting tool names guards nothing. The only safety boundary that holds reads the statement, not the tool: enumerate the safe verbs and default-deny everything else."
+++
