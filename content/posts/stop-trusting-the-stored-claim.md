...
slug = 'stop-trusting-the-stored-claim'
...
tldr = "A decision record that says a deterministic step ran is not proof it ran. The blog lander stopped believing the producer's receipt and started running the pattern engine itself over the still-uncommitted record, which is safe precisely because the record is uncommitted. The same move showed up the same day in a Snowflake operator pack and in a repository truth audit."
+++
...
- [Bind the Receipt to the Commit It Installed](/posts/the-commit-the-test-actually-installed/)
