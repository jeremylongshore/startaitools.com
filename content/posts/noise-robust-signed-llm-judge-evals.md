...
toc = true
tldr = "An un-seeded LLM judge is a coin flip even at temperature 0, so signing a single-call verdict into a transparency log attests noise as if it were truth. The fix: sign a deterministic fold over N recorded votes, separate judge noise from execution-sampling noise, gate blockers on measured agreement, and label replay-fidelity (RF-1) honestly. Proof: a skill that flip-flopped 6 BLOCK / 1 SHIP now reproduces 7/7 — signed, Rekor-anchored, and stranger-verifiable, including an honest BLOCK of our own skill."
+++
