...
1. **Backup growth: ADDRESSED 2026-09-05/06** (intent-os PR #580 + the dev-box script). Causes named by `borg diff`: backing up backups (TeamKB's nightly encrypted tarballs, 283 MB/night, zero deduplication, in both repos), 90 retained tonsofskills releases (27.9 GB) on the VPS, and ~17 GB of rebuildable toolchains and caches on the dev box. Fixed by excludes on both repos and a keep-10 release retention in the deploy; 78 stale releases pruned (24.5 GB); proving run 110.5 GB → 93.5 GB per archive. Existing repos shrink as archives age out. Original text follows for the record:** Backup growth is the load-bearing problem and is out of this repo's hands.** `/backup` (92 G) and `~/backups` (22 G) need a decision from the backup owner: re-measure the intent-os backup README numbers, explain why the VPS repo doubled since August, and set a size budget. Re-homing the B2 origin to the home server is already decided (intent-os decision-log/043) and would move the replica off this disk.
