...
- **The Vibe Marketing pack is installed globally, never vendored.** The 11 skills live at `~/.claude/skills/` as interactive tools for marketing surfaces across the properties. They are deliberately **not** wired into the automated blog pipeline (whose register is the work journal), and their prose is never a writing exemplar: those reference files carry 378 em dashes against Invariant #1. A CI test (`tests/test_blog_pipeline.py::test_no_vibe_derived_text_in_the_repo`) fails the build if that pack's text or vendor namespace appears in any tracked file. The exact marker strings live in `VENDOR_MARKERS` in that test and deliberately are **not** restated here, because restating them is itself a hit (which is how the guard first fired, on this very line).
