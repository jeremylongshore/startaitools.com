...
tldr = "Claude Tag ships a genuinely strong agent into Slack — sandbox, credential proxy, deny-by-default egress, central admin governance, compounding memory — with zero infrastructure. The trade you accept: the substrate is Anthropic's. The sandbox, the memory, and the audit log run on their infra and under their control; governance is coarse (channel scope + spend caps) and the record is theirs to show you, not yours to verify. CCSC and AGP make the opposite trade: fine-grained deterministic policy, per-tool-call human approval, and a signed, offline-verifiable audit you check with your own public key — no vendor in the trust path. Both are legitimate. Pick by who must own the substrate."
+++
