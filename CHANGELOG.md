...
- Keep shared front-matter verification read-only by compiling the existing parser without import-created bytecode. The real CLI regression removes the ambient `PYTHONDONTWRITEBYTECODE` flag so tests cannot hide out-of-scope helper files; preserve isolated write-set enforcement. Corrects d8c810a7/90d1548c, owner#73.
- Order deployment after verified live release publication instead of racing the source push. Publish the verified master commit and annotated tag atomically through normal Git protection; require the GitHub release to exist before handing its identity to the unchanged pinned deployment path. Recover exact generated metadata commits after a failed release API boundary without a new bump or accepting unrelated source; reject authentication/quota/malformed responses and keep dry/no-work behavior explicit. Real local Git regressions reproduce the prior version drift and prove ref refusal, retry and pre/post deployment guards. The central forced-SSH transport remains master-only; the next real host/version rollout still requires production evidence. Owner#83.
- Recognize native CLI task completions delivered as typed `queued_command` attachments during a turn, alongside existing SDK-origin user notifications. Require the parent session, exact envelope, source UUID/time metadata and all existing call/Agent/output/status/result bindings; queue operations, quoted prompts and manual result reads cannot attest completion. Old944 fixtures reproduce ignored valid callbacks and misclassified failures. The observed producer's separate quota failure remains a rejected process outcome, not recovered publication. Owner#73.
- Publish retirement crash-fixture readiness atomically so the parent cannot observe an empty JSON file between creation and write. Preserve all descendant expiry and three-lock assertions; production retirement behavior is unchanged. Owner#81.
- Execute failure-exit regressions through the real leased workspace runner with isolated child processes and persisted outcome assertions. The older extracted-function harness omitted the new runner/manifest and failed four CI cases before testing the provider status; preserve every failure assertion and the production acceptance guard. Owner#73.
- Require the producer function to return success after both process exit0 and full contract verification before invoking the lander. All producer modes persist run/date/source-bound started/completed process attempts under the run lease; new quality seals reject unknown, interrupted, nonzero, malformed or terminal outcomes even after a wrapper crash. Preserve already verified historical seals without inventing process history. Rejected producers retain workspace/native evidence through quarantine, report failure and withhold `.ok`; existing-public no-op and publication recovery remain independent. Actual wrapper regressions reproduce prior lander invocation after exit143, timeout124 and incomplete exit0 output; direct-seal regressions cover durable rejection. Invocation alone is not evidence of publication. Owner#73.
- Distinguish native asynchronous Agent launch from completion. Claude2.1.274 can launch asynchronously without an explicit background flag; only a later SDK-origin notification bound to session, tool call, Agent and output file supplies completed output. Pending, quoted, mismatched, failed or cancelled work blocks verification and even duplicate authority append. Native hard-gate receipts come from completed output, never launch text. Owner#73.
- Validate the complete staged classifier/audit pair, final post bytes, real native Agent completions and, for manifest-bound production, the held run lease before either decision append, including duplicate appends. A real recovery appended authority before discovering four undispatched Tier2 SEO reviews; the supported old CLI regression reproduces that mutation. Pending work now blocks authority writes. Normal completion and preflight require explicit Boolean `draft=false` and exact target slug/date. Owner#73.
- Bound completed isolated-checkout retention through durable evidence, owned-run verification and crash recovery, keeping two eligible checkouts for at least24hours. Quarantine, unfinished delivery, active children, foreign changes and uncertain local Beads data stay protected. Surviving retirement Git children retain all three locks after parent termination; an independent 60-second GNU watchdog and 5-second KILL grace bound even adopted descendants after a successful Git leader exit. Crash regressions verify autonomous three-lock recovery without manual child release; bounded unchanged-subset recovery resumes afterward. This is defensive hardening, not an attributed production-hang cause. Require estimated checkout/Hugo capacity plus500MiB before allocation and report the external registry alongside original owner quarantine. Owner#81.
- Fetch complete source history in the Python CI job so executable regressions run the actual earlier verifier and CLI implementations.
- Select only the true run-scoped classifier during landing; a tiered audit is not classification authority. Preserve validated records, refuse missing/stale bound receipts instead of healing them, expose engine errors and stop replaying historical warnings into current status. Independently verify complete deterministic pattern output before append and completion, including exact numeric/Boolean types. Owner#82.
...
- Rebuild the canonical methodology index from bounded, committed source snapshots at authoritative origin/master through a private Git object view and the existing atomic validator. Do not rebuild only an isolated producer index or admit unpublished/quarantined JSONL. Require that reconciliation even when an existing public article skips generation; healthy no-op cannot bypass a missing/stale index. Preserve the prior canonical index and owner worktree on failure.
...
## Changes since v1.17.28

- chore: release v1.17.29 [skip ci] (47edb766)
- chore(methodology): weekly feedback-sweep 2026-09-13 (8e140e69)
...
## Changes since v1.17.14

- chore: release v1.17.15 [skip ci] (67200334)
- chore(methodology): weekly feedback-sweep 2026-09-06 (1b3eedf5)
...
- chore: release v1.14.3 [skip ci] (2cd15881)
- chore(methodology): weekly feedback-sweep 2026-08-30 (c411ac04)
...
- chore: release v1.11.7 [skip ci] (89ffaa2b)
- chore(methodology): weekly feedback-sweep 2026-08-16 (9fa6c460)
...
one-line sections below are auto-generated by `release.yml` on every push.

### Added
- Three recovered daily posts after two silently missed days:
  "The Version Number That Only Existed on the Client" (2026-08-01, Tier 2),
  "When Live Numbers Argue Back" (2026-08-02, Tier 1),
  "The Check That Only Confirmed a Name" (2026-08-03, Tier 2) — each with
  full classifier records, quality gates, and audit addenda in
  `methodology/decisions.jsonl`.

### Fixed
- Daily pipeline preflight aborted whole runs when only the deliberately
  carried `.beads/interactions.jsonl` was dirty: local `pull.rebase=true`
  routed `git pull --ff-only` through the rebase path. Forced the merge
  path in code (`-c pull.rebase=false`) — root cause of the missed
  2026-08-01 post.
- Completed the estate Buzz alert-floor cutover in all 12 `scripts/blog/`
  cron scripts; the previously uncommitted refactor had tripped the
  dirty-tree guard and blocked the 2026-08-02/03 runs.
- Restored GitHub → VPS auto-deploy: minted the per-repo Tailscale
  workload-identity trust, set the missing repo secrets, verified
  push-triggered deploys green end-to-end. Also fixed root-owned
  `.git`/`dist` objects on the VPS that broke even manual deploys.

### Changed
- README deployment/hosting sections now describe the VPS production
  contract (Netlify demoted to rollback-only); CLAUDE.md automation
  times synced to the live crontab; post counts refreshed (329).
- Dependabot: `actions/checkout` → 7.0.1, `actions/setup-python` → 7.0.0.

### Security
- Confirmed all three historical Tailscale credentials in the archived
  runbook history are revoked (closes the open verification from
  intent-os doc 096); refreshed API credential stored encrypted in sops.
